...
centroids <- tilemaps:::transform_centroids(us_3857$geometry, neighbors, crs, s, prop = 0.1)
...
transformed_boundary <- tilemaps:::transform_boundary(us_3857$geometry,
                                                      centroids$noisy_centroids,
                                                      interpolated_centroids)
smoothed_boundary <- smoothr::smooth(transformed_boundary, method = "ksmooth",
                                     smoothness = 20)

transformed_coords <- st_coordinates(st_boundary(transformed_boundary))
smoothed_coords <- st_coordinates(st_boundary(smoothed_boundary))
boundary <- c(rep("transformed", nrow(transformed_coords)),
              rep("smoothed", nrow(smoothed_coords)))
boundary_df <- data.frame(rbind(transformed_coords, smoothed_coords), boundary)

ggplot() +
  geom_sf(aes(geometry = us_3857$geometry)) +
  geom_path(data = boundary_df, aes(X,Y, group = L1, color = boundary)) +
  theme(axis.text = element_blank(),
        axis.ticks = element_blank(),
        axis.title = element_blank())
```
...
```{r echo=FALSE}
tiles <- tilemaps:::fit_tiles(smoothed_boundary, nrow(us_3857),
                              s, shift = c(0.5,0.5))
```
...
permutation <- tilemaps:::assign_regions(interpolated_centroids, st_centroid(tiles))
